alter table public.point_classifications
...
drop function if exists public.list_point_classifications(boolean);
create function public.list_point_classifications(p_only_active boolean default true)
returns table (
  id uuid,
  slug text,
  name text,
...
as $$
  ...
  where not coalesce(p_only_active, true)
     or pc.is_active
  group by
    pc.id,
    pc.slug,
...
grant execute on function public.list_point_classifications(boolean) to anon, authenticated, service_role;
